...
var ending = 0;
var src = ["assets/media/vial/info_vial_", "assets/media/leader-answer/leader_", "assets/media/vial/leader_vial_", "assets/media/info-answer/info_selected_","assets/media/marker/marker_"];
...
function init() {
  ...
  for (let i = 0; i < NUM_OF_LEADERS; i++) {
    ...
    $("#names-flexbox").append(`<div class="div_leader${i + 1} div-all-leader"><img id="leader-${i+1}" data-num=${i+1} class="leaders" src="assets/media/leader-answer/leader_${i+1}.svg"><p class="p-leaders">${arrLeaders[i].name}</p></div>`);
  ...
  $("#about").on("click", function() {
    ...
    $("#notebook-scrollbar").html(`<div id="about-title">אודות</div><div id="about-content"></div>`);
    audio.pause();
    audio.currentTime = 0;
  });
  // game
...
function reaction(reactionKind) {
  ...
  setTimeout(function() {
    if (reactionKind === "right") {
      ...
      // debugger;
      // document.getElementById(`div_leader${selectedleader}`)
      $(".div_leader" + selectedleader).css("display", "none");
      ...
      if (ending === NUM_OF_LEADERS) {
        ...
        setTimeout(function() {
          ...
          $("#speech-bubble").html(`עזרתם לי להכין את כל התרופות!<br><span id="great-job-span">עבודה מעולה!</span><br><br>הקוד הוא ${CODE}`);
